...
powershell -command "Add-VpnConnection -ServerAddress '%server_addr%' -Name '%conn_name%' -AllUserConnection -TunnelType IKEv2 -AuthenticationMethod MachineCertificate -EncryptionLevel  Required -Splittunneling -PassThru"
if !errorlevel! neq 0 (
...
powershell -command "add-vpnconnectionroute -connectionname '%conn_name%' -destinationprefix 10.5.10.0/27  -passthru"
if !errorlevel! neq 0 (
  echo ERROR: Could not set IPsec Routing configuration for the IKEv2 VPN connection.
  goto :Done
)
